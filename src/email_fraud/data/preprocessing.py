...
# Corporate disclaimer / boilerplate anchors — applied in preprocess() when strip_boilerplate=True.
...
def _is_usable(body: str, min_chars: int, min_words: int, min_alnum_ratio: float) -> bool:
    ...
    2. Minimum word count — catches symbol-only or stub bodies.
    3. Placeholder ratio — catches emails mostly stripped to entity tokens.
    4. Alphanumeric ratio — catches delimiter rows, ASCII tables, structural noise.
    """
    stripped = body.strip()
